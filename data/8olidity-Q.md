## Create3 does not use the create2 opcode

Write in the create contract comment

```solidity
CreateDeployer contract using the CREATE2 opcode and then utilizes the CreateDeployer
```

But in fact, for the creation of new contracts, the create opcode is still used

```solidity
deployed := create(0, add(bytecode, 32), mload(bytecode))
```

Here is a copy of the library code that uses the Create2 opcode

```solidity
library Create3 {
	error ErrorCreatingProxy();
	error ErrorCreatingContract(bytes);
  ...
  */
	function create3(bytes32 _salt, bytes memory _creationCode) internal returns (address addr) {
		...
		if (!success || codeSize(addr) == 0) revert ErrorCreatingContract(data);
	}
  ...
  */
	function addressOf(bytes32 _salt) internal view returns (address) {
		address proxy = address(uint160(uint256(keccak256(abi.encodePacked(hex"ff", address(this), _salt, KECCAK256_PROXY_CHILD_BYTECODE)))));
